...
import { fetchGallery, calculateSelectionStats, getOriginalUrl } from '../lib/selection-api';
import { Gallery, SelectionFilter } from '../lib/selection-types';
...
export default function SelectionResults() {
  ...
  const exportSelections = () => {
    const selected = gallery.photos?.filter((p) => p.selected === 1) || [];
    const text = selected
      .map(
        (p, i) =>
    ...
    const blob = new Blob(
      [
        ...
        `\nSelected Photos (${selected.length}/${gallery.photos?.length || 0}):\n\n`,
        text,
      ],
      { type: 'text/plain' }
    );
  ...
  const copyOriginalLinks = () => {
    const selected = gallery.photos?.filter((p) => p.selected === 1) || [];
    const links = selected.map((p) => getOriginalUrl(p.url)).join('\n');
    navigator.clipboard.writeText(links);
    showToast('Original High-Res links copied! 🔗');
  };
  ...
  const downloadCopyScript = () => {
    ...
    if (filenames.length === 0) {
      showToast('No photos selected to copy!');
      return;
    }

    // Windows Batch Script (.bat) with wildcard extension support
    const scriptContent = [
      '@echo off',
      'echo Selection Tool: High-Res Copy Script',
      'echo ----------------------------------',
      'mkdir Selected_Photos 2>nul',
      'set count=0',
      '',
      ...filenames.map(f => {
        const nameWithoutExt = f.replace(/\.[^/.]+$/, "");
        return `if exist "${nameWithoutExt}.*" ( copy "${nameWithoutExt}.*" "Selected_Photos\\" & set /a count+=1 ) else ( echo [NOT FOUND] ${nameWithoutExt} )`;
      }),
      '',
      'echo ----------------------------------',
      'echo DONE! Checked for files. Files copied to "Selected_Photos" folder.',
      'echo If some files were NOT FOUND, check if they are in this folder.',
      'pause'
    ].join('\r\n');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `copy-selected-${gallery.name.replace(/\s+/g, '-').toLowerCase()}.bat`;
    a.click();
    URL.revokeObjectURL(url);
    showToast('Batch Script Downloaded! 📁');
  };
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                title="Download a Windows script to copy high-res originals locally"
                className="px-4 py-2.5 rounded-xl glass glass-hover text-sm flex items-center gap-2 text-primary hover:text-primary transition-all border border-primary/20"
              >
                <Terminal className="w-4 h-4" />
                Copy Selected Script
              </button>
              <button
                onClick={copyOriginalLinks}
                className="px-4 py-2.5 rounded-xl glass glass-hover text-sm flex items-center gap-2 text-muted-foreground hover:text-foreground transition-all"
              >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            {(
              [
                ...
                { key: 'all', label: 'All', icon: ImageIcon, count: stats.total },
              ] as const
            ).map((btn) => (
              <button
                key={btn.key}
              ...
              {filteredPhotos.map((photo) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <a 
                      href={getOriginalUrl(photo.url)} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="p-2 rounded-lg bg-white/20 hover:bg-white/40 text-white transition-all scale-90 group-hover:scale-100 backdrop-blur-sm"
                      title="View High Res"
                    >
                      <ExternalLink className="w-4 h-4" />
                    </a>
                    <a 
                      ...
                      className="p-2 rounded-lg bg-primary/80 hover:bg-primary text-white transition-all scale-90 group-hover:scale-100 shadow-lg"
