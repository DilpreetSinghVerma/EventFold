...
import { fetchGalleries, deleteGallery, calculateSelectionStats } from '../lib/selection-api';
import { Gallery } from '../lib/selection-types';
...
export default function SelectionDashboard() {
  ...
  const { data: galleries = [], isLoading } = useQuery<Gallery[]>({
    queryKey: ['/api/selection/galleries'],
  ...
  const handleCopyLink = (id: string) => {
    const link = `${window.location.origin}/selection/gallery/${id}`;
    navigator.clipboard.writeText(link);
    showToast('Link copied to clipboard! 📋');
  };

  const handleCreateDemo = () => {
    showToast('Real-time database sync active! Please use Create Gallery for new projects.');
  };
        ...
        >
          <div>
            <h1 className="text-4xl md:text-5xl font-display font-bold tracking-tight">
              <span className="gradient-text">Selection Dashboard</span>
            </h1>
        ...
        {galleries.length === 0 ? (
          ...
          <motion.div variants={fadeInUp} className="space-y-4">
            {galleries.map((gallery) => {
              ...
              const status = statusConfig[gallery.status as keyof typeof statusConfig];
              const progress =
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <Link
                        to={`/selection/gallery/${gallery.id}`}
                      ...
                      <Link
                        to={`/selection/results/${gallery.id}`}
